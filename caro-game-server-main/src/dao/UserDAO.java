...
public class UserDAO extends DAO {
    ...
    public User verifyUser(User user) {
        try {
            ...
            if (rs.next()) {
                ...
                if (PasswordUtils.decrypt(encryptedPassword).equals(user.getPassword())) {
    ...
    public List<User> getListFriend(int ID) {
        ...
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT User.ID, User.NickName, User.IsOnline, User.IsPlaying " +
                    "FROM [user] " +
                    "WHERE User.ID IN (SELECT ID_User1 FROM friend WHERE ID_User2 = ?) " +
    ...
    public boolean checkIsFriend(int ID1, int ID2) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT ID_User1 FROM friend WHERE (ID_User1 = ? AND ID_User2 = ?) OR (ID_User1 = ? AND ID_User2 = ?)");
    ...
    public int getRank(int ID) {
        int rank = 1;
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT ID FROM [user] ORDER BY (NumberOfGame + numberOfDraw * 5 + NumberOfWin * 10) DESC");
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
                if (rs.getInt(1) == ID)
                    return rank;
    ...
    public List<User> getUserStaticRank() {
        ...
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT * FROM [user] ORDER BY (NumberOfGame + numberOfDraw * 5 + NumberOfWin * 10) DESC LIMIT 8");
            System.out.println(preparedStatement);
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
    ...
    public int getNumberOfWin(int ID) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT NumberOfWin FROM [user] WHERE ID = ?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()) {
                return rs.getInt(1);
    ...
    public int getNumberOfDraw(int ID) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT NumberOfDraw FROM [user] WHERE ID = ?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()) {
                return rs.getInt(1);
    ...
    public void addDrawGame(int ID) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("UPDATE [user] SET NumberOfDraw = ? WHERE ID = ?");
    ...
    public void addWinGame(int ID) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("UPDATE [user] SET NumberOfWin = ? WHERE ID = ?");
    ...
    public int getNumberOfGame(int ID) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT NumberOfGame FROM [user] WHERE ID = ?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()) {
                return rs.getInt(1);
    ...
    public void addGame(int ID) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("UPDATE [user] SET NumberOfGame = ? WHERE ID = ?");
    ...
    public String getNickNameByID(int ID) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT NickName FROM [user] WHERE ID = ?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()) {
