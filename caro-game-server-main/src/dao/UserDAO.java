...
public void addUser(User user) {
    ...
    public boolean checkIsFriend(int ID1, int ID2) {
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT Friend.ID_User1 FROM friend WHERE (ID_User1 = ? AND ID_User2 = ?) OR (ID_User1 = ? AND ID_User2 = ?)");
    ...
    public List<User> getUserStaticRank() {
        ...
        try {
            PreparedStatement preparedStatement = con.prepareStatement("SELECT * FROM [user] ORDER BY (user.NumberOfGame + user.numberOfDraw * 5 + user.NumberOfWin * 10) DESC LIMIT 8");
            System.out.println(preparedStatement);
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
